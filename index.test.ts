...
describe('px2rem', () => {
  it('should work on the readme example', () => {
    ...
    const processed = postcss(px2rem()).process(input).css;

    expect(processed).toBe(output);
  });

  it('should replace the px unit with rem', () => {
    const processed = postcss(px2rem()).process(basicCSS).css;
...
describe('ignoreIdentifier', () => {
  it('should not replace px when ignoreIdentifier enabled', () => {
    ...
    const processed = postcss(px2rem(options)).process(input).css;
...
describe('rpx support', function () {
  ...
  it('should replace rpx in media queries', () => {
    ...
    const processed = postcss(px2rem(options)).process('@media (min-width: 500rpx) { .rule { font-size: 16px } }').css;
...
describe('functional options support', () => {
  it('should work when rootValue option use function value', () => {
    const input = 'h1 { margin: 0 0 20px 20px; font-size: 32px; line-height: 1.2; letter-spacing: 1px; }';
    const output = 'h1 { margin: 0 0 0.2rem 0.2rem; font-size: 0.32rem; line-height: 1.2; letter-spacing: 0.01rem; }';
    const output2 = 'h1 { margin: 0 0 0.4rem 0.4rem; font-size: 0.64rem; line-height: 1.2; letter-spacing: 0.02rem; }';
    const postcssInstance = postcss(px2rem({
      rootValue: (root) => {
        return root.source?.input.file?.includes("vant") ? 50 : 100
      }
    }))
    const processed = postcssInstance.process(input, { from: 'main.css' }).css;
    const processed2 = postcssInstance.process(input, { from: 'vant/main.css' }).css;
    expect(processed).toBe(output)
    expect(processed2).toBe(output2)
  })
})
